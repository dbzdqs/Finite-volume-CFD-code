...
!////////////////////////////////////////////////////////////////////////////////////////////!
!*********************************************************************************************
 Subroutine ConMeanFlow_AUSM_ALE(Dim,NC,NF1,NF2,NF,IDS,NX,NY,DA,GM,GF,WNP1,WB,P,Con)
 Implicit None
!*********************************************************************************************
 Intent(In   )::Dim,NC,NF1,NF2,NF,IDS,NX,NY,DA,GM,WNP1,WB,P,GF
 Intent(Out  )::Con
...
!Part 2:
 DO I=NF2+1,NF
   ...
   !Part 6:
    F1 = (Q - GF(I) ) *  Wb(1,I) 
    F2 = (Q - GF(I) ) *  Wb(2,I) + Pm*NX(I)
    F3 = (Q - GF(I) ) *  Wb(3,I) + Pm*NY(I)
    F4 = (Q - GF(I) ) *  Wb(4,I) + Pm* Q 
...
!Part 8:
 DO I=NF1+1,NF2
   ...
   !Part 11:
    ...
    M_L = ( (WNP1(2,L)*NXX+WNP1(3,L)*NYY)/WNP1(1,L) - (GF(I)/DAA) ) / a_L
    M_R = ( (WNP1(2,R)*NXX+WNP1(3,R)*NYY)/WNP1(1,R) - (GF(I)/DAA) ) / a_R
 
   !Part 12:
    IF(Dabs(M_L)>1.)Then
        M_Plus = 0.5*(M_L+Dabs(M_L))
        P_Plus = 0.5*(M_L+Dabs(M_L))/Dabs(M_L)
    Else
   ...
   !Part 13:
    IF(Dabs(M_R)>1.)Then
        ...
        P_Minus = 0.5*(M_R-Dabs(M_R))/Dabs(M_R)
    Else
